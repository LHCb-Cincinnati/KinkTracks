...
Date: 2023-03-20
Usage: lb-run DaVinci/v45r8 python3 -i neutralino_hits.py
'''
...
IOHelper().inputFiles(['/afs/cern.ch/work/m/melashri/public/SUSY/MC/Sim10/Gauss_Dev/GaussDev_v55r4/Neutralino.sim'], clear=True)
...
velo_muon_hit_count_only = 0
tt_muon_hit_count_only = 0
it_muon_hit_count_only = 0
tt_muon_hit_count_only = 0
rich_muon_hit_count_only = 0
muon_muon_hit_count_only = 0
ot_muon_hit_count_only = 0
...
while processed < evtmax:
    ...
    for location in mchits_locations: # Inner loop 1: iterate through the MCHits locations for each subdetector
        ...
        for mchit in mchits: # Inner loop 2: iterate over the MCHits within a specific subdetector
            ...
            if particle_id == 1000022 or particle_id == -1000022:
                neutralino_hit_count += 1
            ...
            ## Now check for which subdetector this muon hit belongs to
                ...
                if mother:
                    ...
                    if mother_id == 1000022 or mother_id == -1000022:
                        interesting_muon_hit_count += 1
            ## Now we want to get hits in each subdetector from muons that are decay products of neutralinos only 
            if particle_id == 13 or particle_id == -13:
                mother = mcparticle.mother()
                if mother:
                    mother_id = mother.particleID().pid()
                    if mother_id == 1000022 or mother_id == -1000022:
                        if location == "/Event/MC/Velo/Hits":
                            velo_muon_hit_count_only += 1
                        elif location == "/Event/MC/TT/Hits":
                            tt_muon_hit_count_only += 1
                        elif location == "/Event/MC/IT/Hits":
                            it_muon_hit_count_only += 1
                        elif location == "/Event/MC/OT/Hits":
                            ot_muon_hit_count_only += 1
                        elif location == "/Event/MC/Muon/Hits":
                            muon_muon_hit_count_only += 1
                        elif location == "/Event/MC/Rich/Hits":
                            rich_muon_hit_count_only += 1             
...
print("Number of Neutralino hits: ", neutralino_hit_count)
...
print("Number of interesting Muon hits (From Neutralino): ", interesting_muon_hit_count)
...
hits_all = [total_velo_hits, total_tt_hits, total_it_hits, total_ot_hits, total_muon_hits, total_rich_hits]
plt.figure()  # Add this line to create a new figure
plt.bar(subdetectors, hits_all, width=0.5, color="blue")
plt.xlabel("Subdetector")
plt.ylabel("Number of hits")
plt.yscale('log')
plt.title("Number of hits in each subdetector")
plt.savefig("figs/neutralino/hits_in_each_subdetector.pdf")

# Create histogram plot of the number of hits from each particle
particles = ["Neutralino", "Muons", "Muons from neutranlino decay"]
hits = [neutralino_hit_count, muons_hit_count, interesting_muon_hit_count]
plt.figure()  # Add this line to create a new figure
...
plt.savefig("figs/neutralino/hits_from_each_particle.pdf")

# Create histogram for number of muons hits in each subdetector
subdetectors = ["Velo", "TT", "IT", "OT", "Muon", "Rich"]
muon_hits = [velo_muon_hit_count, tt_muon_hit_count, it_muon_hit_count, ot_muon_hit_count, muon_muon_hit_count, rich_muon_hit_count]
plt.figure()  # Add this line to create a new figure
plt.bar(subdetectors, muon_hits, width=0.5, color="green")
plt.xlabel("Subdetector")
plt.ylabel("Number of muon hits")
plt.yscale('log')
plt.title("Number of muon hits in each subdetector")
plt.savefig("figs/neutralino/muon_hits_in_each_subdetector.pdf")


# Create histogram for number of muons that are decay from neutralinos hits in each subdetector
muon_hits_only = [velo_muon_hit_count_only, tt_muon_hit_count_only, it_muon_hit_count_only, ot_muon_hit_count_only, muon_muon_hit_count_only, rich_muon_hit_count_only]
plt.figure()  # Add this line to create a new figure
plt.bar(subdetectors, muon_hits_only, width=0.5, color="orange")
plt.xlabel("Subdetector")
plt.ylabel("Number of muon (from neutralino) hits")
plt.yscale('log')
plt.title("Number of muon hits (from neutralino) in each subdetector")
plt.savefig("figs/neutralino/muon_hits_from_neutralino_in_each_subdetector.pdf")
